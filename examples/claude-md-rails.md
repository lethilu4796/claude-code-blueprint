...
- Follow Rails conventions -- don't fight the framework. Use the standard directory structure, naming conventions, and file locations
- Use Active Record migrations for all schema changes -- never modify the database directly
- Use Strong Parameters in controllers -- never use `params.permit!` (mass assignment vulnerability)
- Use model validations for business rules -- never validate only in the controller
- Use `rspec` with FactoryBot for testing -- every model and controller action needs at least one spec
- Use Rubocop for linting -- follow the project's `.rubocop.yml` or Rails default rules
- Background jobs use Sidekiq or Solid Queue -- never do long-running work in a request cycle
- Soft delete only -- use `acts_as_paranoid` or a `discarded_at` column. Never hard DELETE records
