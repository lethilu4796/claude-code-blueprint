...
- Use the standard library (`net/http`, `encoding/json`) for simple APIs. Use Gin or Echo only for projects that need middleware chains or route groups
- Return errors, don't panic -- `panic` is for truly unrecoverable situations only. Every function that can fail returns `error`
- Use `sqlx` or `GORM` for database access -- never use `database/sql` directly without a query builder in production code
- Use Go modules (`go.mod`) for dependency management -- never vendor dependencies unless required by deployment constraints
- Use table-driven tests with `testing.T` -- every exported function needs at least one test case
- Use `golangci-lint` with the default configuration -- it catches most common Go mistakes
- Struct fields that are optional in API responses should use pointer types (`*string`, `*int`) -- never use zero values to mean "not set"
- Context propagation is mandatory -- pass `context.Context` as the first parameter to any function that does I/O
