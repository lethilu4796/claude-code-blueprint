...
- Use Next.js App Router (not Pages Router) for new projects -- server components by default, `"use client"` only when needed
- Use TypeScript strict mode -- never use `any` as a type. If a type is complex, define an interface
- Use React Server Components for data fetching -- avoid `useEffect` for loading data on mount
- Use Tailwind CSS for styling -- no CSS modules, styled-components, or inline styles unless the project already uses them
- Use Zod for runtime validation of API responses and form inputs -- TypeScript types don't exist at runtime
- Use `next/image` for all images -- never use raw `<img>` tags (missing optimization)
- State management: use React context for UI state, server components for server state. Only add Zustand/Jotai if context becomes deeply nested
- All API routes follow REST conventions -- GET for reads, POST for creates, PATCH for updates, DELETE for soft-deletes
